...
import { M as useMediaQuery, j as useLocalStorage } from "./_MapCache-It4eafc2.js";
import { Js as useRootStore, Li as useUIStore, gc as defineStore, ir as useUsersStore, lc as streamRequest, sc as makeRestApiRequest, yc as v4_default } from "./useTelemetry-D6pZULgL.js";
import { po as CHAT_HUB_SIDE_MENU_DRAWER_MODAL_KEY, qo as LOCAL_STORAGE_CHAT_HUB_STATIC_SIDEBAR, ur as chatHubProviderSchema } from "./constants-UStNMe6H.js";
import { t as retry } from "./retry-sDkwzrPY.js";
import { i as MOBILE_MEDIA_QUERY, n as CHAT_STORE } from "./constants-DlQs6Cav.js";
...
const useChatStore = defineStore(CHAT_STORE, () => {
	...
	async function onStreamDone(sessionId) {
		streamingMessageId.value = void 0;
		await retry(async () => {
	...
	function sendMessage(sessionId, message, model, credentials) {
		...
		sendMessageApi(rootStore.restApiContext, {
			...
			credentials,
			previousMessageId
		}, (chunk) => onStreamMessage(sessionId, chunk, replyId, messageId, null), async () => await onStreamDone(sessionId), onStreamError);
	}
	...
	function regenerateMessage(sessionId, retryId, model, credentials) {
		...
		regenerateMessageApi(rootStore.restApiContext, sessionId, retryId, {
			...
			replyId,
			credentials
		}, (chunk) => onStreamMessage(sessionId, chunk, replyId, previousMessageId, retryId), async () => await onStreamDone(sessionId), onStreamError);
	}
