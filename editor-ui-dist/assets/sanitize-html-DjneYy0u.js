	...
	(function(root, factory) {
		...
		return function(input) {
			...
			function parseDescriptors() {
				var pError = false, w, d, h, i, candidate = {}, desc$1, lastChar, value, intVal, floatVal;
				for (i = 0; i < descriptors.length; i++) {
					desc$1 = descriptors[i];
					lastChar = desc$1[desc$1.length - 1];
					value = desc$1.substring(0, desc$1.length - 1);
				...
				} else if (console && console.log) console.log("Invalid srcset descriptor found in '" + input + "' at '" + desc$1 + "'.");
			}
		};
	});
}));
...
var require_path_browserify = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var posix = {
		...
		normalize: function normalize(path) {
			...
			var isAbsolute$1 = path.charCodeAt(0) === 47;
			var trailingSeparator = path.charCodeAt(path.length - 1) === 47;
			path = normalizeStringPosix(path, !isAbsolute$1);
			if (path.length === 0 && !isAbsolute$1) path = ".";
			if (path.length > 0 && trailingSeparator) path += "/";
			if (isAbsolute$1) return "/" + path;
			return path;
		},
		isAbsolute: function isAbsolute$1(path) {
			assertPath(path);
		...
		dirname: function dirname$2(path) {
			assertPath(path);
			if (path.length === 0) return ".";
		...
		format: function format$1(pathObject) {
			if (pathObject === null || typeof pathObject !== "object") throw new TypeError("The \"pathObject\" argument must be of type Object. Received type " + typeof pathObject);
			return _format("/", pathObject);
		},
		parse: function parse$7(path) {
			assertPath(path);
			var ret = {
				root: "",
				dir: "",
			...
			if (startDot === -1 || end === -1 || preDotState === 0 || preDotState === 1 && startDot === end - 1 && startDot === startPart + 1) {
				if (end !== -1) if (startPart === 0 && isAbsolute$1) ret.base = ret.name = path.slice(1, end);
				else ret.base = ret.name = path.slice(startPart, end);
			} else {
				if (startPart === 0 && isAbsolute$1) {
		...
		function decode$2(input) {
			...
			for (index = basic > 0 ? basic + 1 : 0; index < inputLength;) {
				for (oldi = i, w = 1, k = base;; k += base) {
					...
					if (digit >= base || digit > floor$1((maxInt - i) / w)) error("overflow");
					i += digit * w;
				...
				if (floor$1(i / out) > maxInt - n) error("overflow");
				n += floor$1(i / out);
				i %= out;
				output.splice(i++, 0, n);
			}
...
var require_object_inspect = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	module.exports = function inspect_(obj, options, depth, seen) {
		...
		if (!isDate(obj) && !isRegExp$1(obj)) {
			...
			var stringTag = !isPlainObject$2 && toStringTag && Object(obj) === obj && toStringTag in obj ? $slice.call(toStr$1(obj), 8, -1) : protoTag ? "Object" : "";
...
var require_input = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var Input$4 = class {
		...
		fromOffset(offset) {
			...
			let min$1 = 0;
			if (offset >= lastLine) min$1 = lineToIndex.length - 1;
			else {
				let max$2 = lineToIndex.length - 2;
				let mid;
				while (min$1 < max$2) {
					mid = min$1 + (max$2 - min$1 >> 1);
					if (offset < lineToIndex[mid]) max$2 = mid - 1;
					else if (offset >= lineToIndex[mid + 1]) min$1 = mid + 1;
					else {
						min$1 = mid;
						break;
					}
			...
			return {
				col: offset - lineToIndex[min$1] + 1,
				line: min$1 + 1
			};
		}
		mapResolve(file) {
...
export { require___vite_browser_external as C, require_path_browserify as S, require_range as _, require_get_intrinsic as a, require_type as b, require_call_bind_apply_helpers as c, require_function_bind as d, require_has_symbols as f, require_syntax as g, require_gopd as h, require_call_bound as i, require_actualApply as l, require_es_define_property as m, empty_exports as n, require_hasown as o, require_shams as p, init_empty as r, require_get_proto as s, require_sanitize_html as t, require_functionApply as u, require_es_object_atoms as v, require_punycode as x, require_object_inspect as y };
