...
import { pt as useCredentialsStore, rn as useTelemetry, s as useWorkflowsStore, t as useUsersStore, tn as useProjectsStore } from "./users.store-Dpbobzbe.js";
import { s as sanitizeHtml, t as useToast } from "./useToast-Pwu_Jqrl.js";
import { Do as createResultOk, Eo as createResultError } from "./constants-B2-iseoM.js";
...
function useCredentialOAuth() {
	...
	*/
	function canOAuthCredentialQuickConnect(credentialTypeName) {
		...
		const visibleProperties = credentialType.properties.filter((prop) => prop.type !== "hidden" && prop.type !== "notice" && !overwrittenProperties.includes(prop.name));
		if (visibleProperties.length === 0) return true;
		if (overwrittenProperties.length === 0) return false;
		return visibleProperties.every((prop) => prop.required !== true || prop.type !== "string" && prop.type !== "number");
	}
...
function useQuickConnect() {
	...
	async function connectToPinecone(quickConnectOption) {
		const { ConnectPopup } = await __vitePreload(async () => {
			const { ConnectPopup } = await import("./dist-BBHztDCP.js");
			return { ConnectPopup };
		}, []);
		return await new Promise((resolve) => {
