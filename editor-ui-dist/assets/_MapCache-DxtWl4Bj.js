...
var zh_CN_default = {
	...
	"workflowSettings.redactionPolicy.options.none": "不进行删改",
	"workflowSettings.redactionPolicy.options.all": "对所有执行进行脱敏处理",
	"workflowSettings.redactionPolicy.options.nonManual": "对非手动执行进行脱敏",
	"workflowSettings.helpTexts.redactionPolicy": "控制执行数据的脱敏方式。'Redact all' 会对所有执行的数据进行脱敏。'Redact non-manual' 仅对生产环境中非手动触发的执行进行脱敏。",
